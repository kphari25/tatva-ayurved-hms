...
const InvoiceModule = ({ supabase, currentUser, userRole }) => {
  ...
  const hospitalDetails = {
    ...
    address: 'Hospital Address Line 1, City, State - PIN',
    phone: '+91 XXXXXXXXXX',
    email: 'contact@tatvaayurved.com',
    gstin: 'GSTIN (if applicable)',
    registrationNo: 'Hospital Registration Number',
    logo: '/logo.png' // Add your logo to public/logo.png
  };
  ...
  const handleSaveInvoice = async () => {
    ...
    try {
      ...
      const invoiceData = {
        ...
        invoice_data: JSON.stringify(newInvoice),
      ...
      if (supabase) {
        ...
        setSelectedInvoice({ ...data, invoice_data: newInvoice });
      }
  ...
  // FIXED PRINT FUNCTION - Opens in new window
  const handlePrint = () => {
    ...
    const printWindow = window.open('', '', 'height=842,width=595'); // A4 size in pixels
    
    printWindow.document.write('<html><head><title>Invoice - ' + (selectedInvoice?.invoice_number || 'New') + '</title>');
              ...
              {invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4">
                    <button
                      onClick={() => {
                        // FIXED: Properly parse JSON data
                        const parsedData = typeof inv.invoice_data === 'string' 
                          ? JSON.parse(inv.invoice_data) 
                          : inv.invoice_data;
                        setSelectedInvoice({ ...inv, invoice_data: parsedData });
                        setView('preview');
                      }}
  ...
  const renderPreviewView = () => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
        ...
        {/* Printable Invoice */}
        <div ref={printRef} id="printable-invoice" className="bg-white max-w-4xl mx-auto shadow-2xl">
          <div className="p-12">
            {/* Header */}
            <div className="header border-b-4 border-blue-600 pb-6 mb-6">
              <div className="flex items-center justify-center gap-6 mb-4">
                ...
                <h1 className="text-4xl font-bold text-blue-600">{hospitalDetails.name}</h1>
              </div>
            ...
            {/* Invoice Details */}
            <div className="invoice-info grid grid-cols-2 gap-8 mb-8">
              <div>
                ...
                <div className="space-y-1 text-sm">
                  ...
                  {invoiceData.patientAge && <p><strong>Age/Gender:</strong> {invoiceData.patientAge} / {invoiceData.patientGender}</p>}
                </div>
              </div>
              <div className="text-right">
                <div className="bg-blue-50 p-4 rounded-lg inline-block">
                  <h3 className="font-bold text-2xl text-blue-600 mb-2">{selectedInvoice.invoice_number}</h3>
                  <p className="text-sm"><strong>Date:</strong> {new Date(invoiceData.invoiceDate || selectedInvoice.invoice_date).toLocaleDateString('en-IN')}</p>
                  <p className="text-sm"><strong>Doctor:</strong> {invoiceData.doctorName || selectedInvoice.doctor_name}</p>
                  {invoiceData.followupDate && (
                    <p className="text-sm text-emerald-600"><strong>Next Visit:</strong> {new Date(invoiceData.followupDate).toLocaleDateString('en-IN')}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            {/* Consultation */}
            <div className="mb-6">
              <h3 className="section-title font-bold text-lg mb-3 pb-2 border-b-2">Consultation</h3>
              <table className="w-full text-sm">
                ...
                <tbody>
                  <tr className="border-b">
                    <td className="p-3">Consultation Fee - Dr. {invoiceData.doctorName || selectedInvoice.doctor_name}</td>
                    <td className="text-right p-3">₹{parseFloat(invoiceData.consultationFee || 0).toFixed(2)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            {invoiceData.treatments && invoiceData.treatments.length > 0 && (
              <div className="mb-6">
                <h3 className="section-title font-bold text-lg mb-3 pb-2 border-b-2">Treatments & Procedures</h3>
                <table className="w-full text-sm">
                  <thead className="bg-slate-100">
                    <tr>
            ...
            {invoiceData.medicines && invoiceData.medicines.length > 0 && (
              <div className="mb-6">
                <h3 className="section-title font-bold text-lg mb-3 pb-2 border-b-2">Medicines & Pharmacy</h3>
                <table className="w-full text-sm">
                  <thead className="bg-slate-100">
                    <tr>
                  ...
                  <tbody>
                    {invoiceData.medicines.map((med, idx) => (
                      <tr key={idx} className="border-b">
                        ...
                        <td className="text-right p-3">₹{(parseFloat(med.quantity || 0) * parseFloat(med.unitPrice || 0)).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="border-t-2 border-slate-300 pt-6">
              <div className="flex justify-end">
                <table className="totals-table w-80">
                  <tbody>
                    <tr>
                      <td className="text-slate-600">Subtotal:</td>
                      <td className="text-right font-semibold">₹{totals.subtotal.toFixed(2)}</td>
                    </tr>
                    {totals.discount > 0 && (
                      <tr className="text-red-600">
                        <td>Discount:</td>
                        <td className="text-right font-semibold">- ₹{totals.discount.toFixed(2)}</td>
                      </tr>
                    )}
                    <tr className="text-emerald-600">
                      <td>GST (18%):</td>
                      <td className="text-right font-semibold">₹{totals.gst.toFixed(2)}</td>
                    </tr>
            ...
            {/* Footer */}
            <div className="footer mt-12 pt-6 border-t-2 text-center text-sm text-slate-600">
  ...
  if (view === 'preview') return renderPreviewView();
